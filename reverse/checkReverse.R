...
function (args) {
    ...
    ## Avoid thread oversubscription: each of the Ncpus parallel workers
    ## below may itself call a threaded BLAS (OpenBLAS/MKL) or OpenMP code
    ## during package installation/checking (e.g. lme4, rstanarm, mmrm).
    ## Left uncapped, a threaded BLAS spawns one thread per core *per
    ## worker*, so Ncpus workers can fan out to Ncpus*ncores threads,
    ## causing heavy context-switch overhead (high %sys, load >> ncores)
    ## instead of real throughput. Cap to 1 BLAS/OMP thread per worker so
    ## the Ncpus worker processes, not intra-process threading, are what
    ## uses the machine's cores.
    if (requireNamespace("RhpcBLASctl", quietly = TRUE)) {
        RhpcBLASctl::blas_set_num_threads(1L)
        RhpcBLASctl::omp_set_num_threads(1L)
    }
    ## RhpcBLASctl only affects this process (and its fork()ed children,
    ## e.g. the mclapply workers spawned below); each package install/check
    ## step below runs in a fresh exec()ed R subprocess, which only
    ## inherits environment variables, not in-process BLAS/OMP state -- so
    ## set the env vars too, covering every BLAS/OpenMP implementation.
    ...
    reverse <- if (length(tarfile) == 1L)
        list(which = c("Depends", "Imports", "LinkingTo",
                       "Suggests", "Enhances"))
    check.vars <-
        c("_R_CHECK_FORCE_SUGGESTS_=false",
          NULL)
    configure.args <-
        list()
    configure.vars <-
