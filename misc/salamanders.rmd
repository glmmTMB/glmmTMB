...
<!-- real = ddply(Salamanders, ~site+spp+mined, summarize, absence=mean(count==0)) -->

<!-- ggplot(newdata, aes(spp, predZ, colour=mined))+geom_point()+ -->
<!--   geom_errorbar(aes(ymin=pmax(predZ.min,0), ymax=pmin(predZ.max,1)))+ -->
...
The easiest thing to do for the point estimates of the unconditional count (`ucount`) is to transform to the response scale and multiply:
...
ggplot(pred.count, aes(x=spp, y=pred.ucount, colour=mined))+geom_point(shape=1, size=2)+
  ...
  xlab("Species")
```
Figure 2 -- Circles represent predicted unconditional counts at the mode (i.e., site effect = 0) and error bars represent the 95% confidence intervals for that mode. Squares represnet the observed median and diamonds represent observed means calculated across samples and sites.
