...
as.theta.vcov <- function(Sigma, lambda = 1, corrs.only = FALSE) {
...
corrs.2 <- as.theta.vcov(mat.small, 0.85, corrs.only = TRUE)
corrs.1 <- as.theta.vcov(mat.small, 1, corrs.only = TRUE)
dat$spp <- factor(rownames(dat), levels=rownames(dat))
dat$dummy <- factor(0)
nsp <- nrow(dat.small)
...
fit.prop <- glmmTMB(matur.L ~ age.mat + prop2( 0 + spp | dummy, mat.small),
        data = dat.small,
        dispformula = ~ 0)
