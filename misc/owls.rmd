...
vignette: >
  %\VignetteIndexEntry{Compare glmmTMB, glmmADMB, MCMCglmm, and brms}
...
##Fit using glmmTMB
```{r tmbfit}
time.tmb = system.time(m1.tmb <- glmmTMB(NCalls~(FoodTreatment + ArrivalTime) * SexParent + 
	offset(logBroodSize) + (1|Nest), ziformula=~1, data = Owls, family="poisson"))
```

##Fit using glmmADMB
```{r admbfit}
time.admb = system.time(m1.admb <- glmmadmb(NCalls~(FoodTreatment + ArrivalTime) * SexParent + 
	offset(logBroodSize) + (1 | Nest), zeroInflation=TRUE, data = Owls, family="poisson"))
```
...
time.mcmc=system.time(m1.mcmc <- MCMCglmm(fixef2,
...
##Fit using brms
```{r brmsfit, results=FALSE, message=FALSE}
time.brms = system.time(m1.brms <- brm(NCalls ~(FoodTreatment + ArrivalTime) * SexParent + 
	offset(logBroodSize) + (1 | Nest), data = Owls, family="zero_inflated_poisson"))
```
...
colnames(coefs$brms)[2:4]=c("Estimate", "LCI","UCI") 
...
ggplot(coefs2, aes(color=.id))+
  ...
  geom_point(aes(y=.rownames, x=Estimate))+
  geom_errorbarh(aes(y=.rownames, x=Estimate, xmax=UCI, xmin=LCI))+
