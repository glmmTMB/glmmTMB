...
I was hoping that `nlme` would have something we could use, but this code (adapted from the C code) gives only the inverse Cholesky factor, not the Cholesky factor ...
...
stopifnot(all.equal(as.matrix(ar1_chol(0.7, 1, 5)),
                    ar1_chol_old(0.7, 1, 5)
                    ))
## closed form expression with factor of ... sqrt(1+1/rho) ?
```

Rcpp version is actually much *slower* for small/realistic-sized problems (breakeven is in the range of `n=250` ...
...
The same approach in `glmmTMB`:

```{r eval = FALSE}
g0 <- glmmTMB(y ~ 1 + (interaction(row, col) | block), data = dd,
              REML = TRUE,
              doFit = FALSE)
## next step dumps core when retaping/setting up model: why??
