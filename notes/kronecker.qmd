...
Schäcke 2004 ([here](https://www.math.uwaterloo.ca/~hwolkowi/henry/reports/kronthesisschaecke04.pdf)) says
...
Let's start by simulating some data (some of the framework here will also be useful in the fitting).  Things 
...
## brute force cmoputation of chol(AR1)
## closed form expression with factor of ... sqrt(1+1/rho) ?
...
```{r ugh, include = FALSE}
## playing with examples to try to guess the form of chol(AR1) ...
ar1_chol(rho = 0.5, sigma = 1, n = 3)  ## cols 2 ... are * sqrt(3)
ar1_chol(rho = 0.75, sigma = 1, n = 3)  ## cols 2 ... are * 6/8
tmpf <- function(rho) {
    a <- ar1_chol(rho = rho, sigma = 1, n = 3)
    a[2,1]/a[2,2]
}
tmpf(0.5)       ## 1/sqrt(3)
tmpf(1/sqrt(2)) ## 1
tmpf(0.8)       ## 4/3
v <- (1:99)/100
plot(v, sapply(v, tmpf), log = "y")
```

```{r utils2}
## generate theta parameters for an AR1 ⨂ AR1 model
get_theta <- function(p) {
    nm <- c("sigma", "rho_R", "rho_C", "n_R", "n_C")
    ## p: sigma, rho_R, rho_C, n_R, n_C
...
## test
rvec <- c(sigma = 2, rho_R = 0.8, rho_C = 0.2, n_R = 10, n_C = 10)
t1 <- get_theta(rvec)
...
```{r fit1}
lmod1 <- lFormula(y ~ 1 + (interaction(row, col) | block),
               data = dd)
## str(f1$reTrms)
...
devfun2 <- function(theta0) {
    ...
    devfun(get_theta(c(theta0, n_R = 10, n_C = 10)))
}
## dev/REMLcrit with original reduced-theta parameters ...
stopifnot(all.equal(d1, devfun2(rvec[1:3])))
opt1 <- nloptwrap(rvec[1:3], devfun2,
                lower = c(0, -1, -1),
                upper = c(Inf, 1, 1))
m1 <- mkMerMod(environment(devfun), opt1, lmod1$reTrms, fr = lmod1$fr)
## show abbreviated summary via capture.output() ?
