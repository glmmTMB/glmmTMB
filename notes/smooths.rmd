...
What is the structure of `smooth_info`? Inside (e.g.) `sm1$modelInfo$reTrms$cond$smooth_info$re`
we have `$rand$Xr` (component of Z matrix) and `$Xf` (component of X matrix); there may (??) be multiple `$Xr` if we have a `by` term??

Code in `getXReTrms` for prediction needs to replicate this structure ...
