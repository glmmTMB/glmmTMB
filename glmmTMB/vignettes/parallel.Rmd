...
Parallelization via OpenMP is **disabled** in the current version of `glmmTMB`, because we haven't worked out the challenges of getting it to compile seamlessly on all platforms. To re-enable OpenMP parallelism, edit `R/glmmTMB.R`, `src/glmmTMB.cpp`, `src/Makevars`, and `tests/testthat/test-control.R` (and this file!); search for "parallel" and "omp.h". See also [this discussion on GitHub]( https://github.com/glmmTMB/glmmTMB/issues/481).
