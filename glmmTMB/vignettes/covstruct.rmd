...
author: "Kasper Kristensen and Maeve McGillicuddy"
date: "`r Sys.Date()`"
...
> FIXME: Explain why dispformula=~1 causes over-overparameterization
...
The left hand side of the bar `taxa + 0` corresponds to a factor loading matrix that accounts for the correlations among taxa. The right hand side of the bar splits the above specification independently among sites. The `n` is a non-negative integer (which defaults to 2). 
...
- `jitter.sd` adds variation to the starting values of latent variables when `start = "res"` (default 0).
