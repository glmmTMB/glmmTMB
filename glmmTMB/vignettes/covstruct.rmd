...
There is a `r round(phi_diff*100)`% difference between the estimated and true (simulated) parameter.
...
Notice that we call `simGroup` `r ngrp` times rather than drawing from a (`r ngrp`*25)-variate normal distribution because observations belonging to different groups are uncorrelated with each other.

Fitting the model on this larger dataset gives estimates closer to
the true values (AR standard deviation=1, residual (measurement) standard deviation=1, autocorrelation=`r phi_sim`):
...
is now about half the true value (`r phi_sim`/2 = `r round(phi_sim/2, 2)`) because the dispersion is
...
amounts to `r (n-1)` correlation parameters. For the heterogeneous version `r n` variance parameters are used, while just a single variance parameter is estimated for the homogeneous version.
