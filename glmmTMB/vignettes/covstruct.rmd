...
```{r predict_all_cache, include=FALSE}
load("covstruct.rda") ## Cached object 'pred'
```

```{r predict_all, eval=FALSE}
...
```{r fit.us.profile.plot, eval=FALSE}
plot(tt)
confint(tt)
```

```{r fit.cs.profile, eval=FALSE}
ppar <- fit.cs$fit$par
length(ppar)
range(which(names(ppar)=="theta")) ## the last n*(n+1)/2 parameters
...
```{r fit.cs.profile.plot, eval=FALSE}
plot(tt2)
```
