...
```{r covstruct-table, echo  = FALSE}
ctab <- read.delim(sep = "|",
                   header = TRUE,
                   check.names = FALSE,
           text = "
 Covariance                       | Notation      | # parameters | Requirement  | Parameters
 Unstructured (general positive definite)      | `us`          |  $n(n+1)/2$     | | See [Mappings]
 Heterogeneous Toeplitz           | `toep`        |  $2n-1$         |              
 Het. compound symmetry  | `cs`          |  $n+1$    |      | log-SDs ($n$); corr (see [Mappings])
 Het. diagonal           | `diag`        |  $n$            |  | log-SDs
 AR(1)                            | `ar1`         |  $2$            | Unit spaced levels | log-SD; $\\rho = \\theta/\\sqrt{1+\\theta^2}$
...
transformation from the parameter ($\phi$) to the AR1 coefficient is
$\phi = theta_2/sqrt(1+\theta_2^2)$; the inverse transformation is
...
unique locations)
- use the information encoded in the levels by `numFactor()` to compute the corresponding distances
