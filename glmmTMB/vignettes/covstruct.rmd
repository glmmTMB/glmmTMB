...
glmmTMB(y ~ ar1(times + 0 | group), data=dd)
```
...
> FIXME: hangs (or very slow) with development TMB + development R?
> `Error in optimHess(par.fixed, obj$fn, obj$gr) : gradient in optim evaluated to length 1 not 13`
> (plus many `NA/NaN function evaluation` warnings from `nlminb`)

```{r fit.toep, eval=FALSE}
...
```{r fit.cs}
fit.cs <- glmmTMB(y ~ cs(times + 0 | group), data=dat1)
...
cc <- diag(n)
cc[lower.tri(cc)] <- cpars
L <- tcrossprod(cc)
D <- diag(1/sqrt(diag(L)))
D %*% L %*% D
## check against internal representation
all.equal(c(cov2cor(vv1)),c(x$obj$env$report(x$fit$parfull)$corr[[1]]))
```

Profiling (experimental/exploratory):

> FIXME: odd results from `fit.us`
