...
author: "Kasper Kristensen, Maeve McGillycuddy, and Coralie Williams"
date: "`r Sys.Date()`"
output: 
...
params:
  ...
  have_metafor: !r require("metafor", quietly = TRUE)
  have_ape: !r require("ape", quietly = TRUE) && require("ade4", quietly = TRUE)
---
...
Homogenous versions of some structures (e.g. compound symmetric) can be implemented by using the `map` argument to set all log-SD parameters equal to each other.
...
- $X(t)$ is a stationary AR(1) process, i.e. has covariance $\textrm{cov}(X(s),
  X(t)) = \sigma^2\exp(-\theta |t-s|)$.
- $\varepsilon(t)$ is independent $N(0,\sigma_0^2)$ measurement error.
...
The following R function draws a simulation based on these parameter values.
Initially, we consider only 1 group observed over 25 time points.

```{r simGroup, eval = TRUE}
...
To fit an AR1 model with `glmmTMB` we must first specify a time
...
data or to order the levels incorrectly. We therefore recommend
constructing factors with explicit levels, using the `levels` argument to
the `factor` function (in the function above we use `levels = 1:n`).
...
(intercept), $\sigma_0^2$ (residual variance), $\sigma^2$ (process variance) and
$\phi$ in the output:
...
By default, the print method for `VarCorr()` prints standard deviations rather than variances: `print(VarCorr(mod), comp = "Var")` 
will print variances instead, or you can inspect the contents of the object (`VarCorr(mod)$cond$group`).
...
<!-- r round(phi_diff*100) -->
There is a 34% difference between the estimated and true (simulated) parameter.
...
Profile confidence intervals will be more accurate:
...
A single time series of 25 time points is not sufficient to identify
the parameters. We could either increase the length of the time series
or increase the number of groups. We'll try the latter, generating a data set with 200 groups:
...
<!-- hard-coded inclusions: "r ngrp", "r ngrp"*25 -->
Notice that we call `simGroup` 200 times rather than drawing from a (`ngrp*25` = 500)-variate normal distribution because observations belonging to different groups are uncorrelated with each other.

Fitting the model on this larger dataset gives estimates closer to
the true values (AR standard deviation=1, residual (measurement) standard deviation=1, autocorrelation=0.7): <!-- r phi_sim -->
...
is now about half the true value (0.7/2 = 0.35) because the dispersion is
<!-- phi_sim, phi_sim/2 -->
...
The standard deviations are all approximately equal to the true total standard
deviation ($\sqrt{\textsub{\sigma^2}{AR} + \textsub{\sigma^2}{meas}}=\sqrt{2}$).

## The Toeplitz structure

The next natural step would be to reduce the number of parameters by
collecting correlation parameters within the same off-diagonal. 
<!-- n-1 , n -->
This
amounts to 24 correlation parameters. For the heterogeneous version 25 variance parameters are used, while just a single variance parameter is estimated for the homogeneous version.
...
Once again, the standard deviations are $\approx \sqrt{2}$
and the elements in the first off-diagonal are $\approx 0.35$.

We can get a *slightly* better estimate of the variance by using REML estimation (however, the estimate of the correlations seems to have gotten slightly worse, and we suppress a "false convergence" warning from `nlminb`):

```{r fit.toep.reml, warning = FALSE}
...
It should give the same results as `ar1` in this case ^[we have actually found that the results from OU and AR1 models are *not* exactly the same; we're digging in to this, please post an issue if this is important to you/you want to know more] since the
...
```{r propto_ex, eval=params$have_ape}
library(ape)
...
The `equalto` structure fits a multivariate random effect using a fixed variance–covariance matrix supplied by the user, no scale or correlation parameters are estimated. This is useful in meta-analysis when sampling-error variances and within-study correlations are known. For example, when there are multiple outcomes (i.e. effect sizes) per study, a block-diagonal variance-covariance matrix can encode the within-study correlations. `equalto` is equivalent to `propto` without the proportionality parameter, that is, the supplied variance-covariance matrix is used as-is.

We use here a meta-analysis dataset from the `metafor` package to demonstrate how `equalto` fits a meta-analysis with within-study dependence among effect sizes:

```{r equalto, message=FALSE, warning=FALSE, eval=params$have_metafor}
library(metafor)
dat <- dat.assink2016
head(dat,9) 
...
colnames(VCV) <- rownames(VCV) <- dat$id
...
We can fit the same model assuming sampling errors are independent:

```{r glmmTMB_meta, eval=params$have_metafor}
...
```{r metafor_models, echo=FALSE, eval=params$have_metafor}
