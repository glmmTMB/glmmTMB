...
The following R-code draws a simulation based on these parameter values.
For illustration purposes we consider only 1 group observed over 25 time points.

```{r sim1, eval=TRUE}
set.seed(1234)                                       ## Set seed for reproducibility
n <- 25                                              ## Number of time points
phi <- 0.4                                           ## AR(1) correlation parameter
sigma <- phi ^ as.matrix(dist(1:n))                  ## var-cov matrix among observations
x <- MASS::mvrnorm(mu = rep(0,n), Sigma = sigma)     ## Draw 1 sample (i.e., 1 group) from a n-variate normal distribution
...
```{r phihat, echo=FALSE}
theta <- glmmTMB::getME(mod, "theta")
phi_hat <- theta[2]/sqrt(1+theta[2]^2)
abs((phi-phi_hat)/phi)                              ## Relative difference between real phi and estimated one
                                                    ## It is large due to the low number of observations
```
...
Notice that we call `simGroup` 1000 times rather than drawing from a (1000*25)-variate normal distribution because observations beloging to different groups are uncorrelated with each other.
...
theta <- glmmTMB::getME(fit.ar1, "theta")
phi_hat <- theta[2]/sqrt(1+theta[2]^2)
abs((phi-phi_hat)/phi)                      ## Much closer!
```
