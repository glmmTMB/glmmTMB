...
where g(.) is the link function, $\boldsymbol{\beta}$ is a p-dimensional vector of regression coefficients related to the covariates, $\boldsymbol{X}$ is an $n \times p$ model matrix, $\boldsymbol{Z}$ is the $n\times q$ model matrix for the $q$-dimensional vector-valued random effects variable, $\boldsymbol{U}$ which follows a multivariate normal distribution with mean zero and a parameterized $q \times q$ variance-covariance matrix, $\boldsymbol{\Sigma}$, i.e., $\boldsymbol{U} \sim N(\boldsymbol{0}, \boldsymbol{\Sigma})$

A general latent variable model (GLVM) requires much fewer parameters for the variance-covariance matrix, $\boldsymbol{\Sigma}$. To a fit a GLVM we want to add a reduced rank (rr) covariance structure, so the model becomes
...
where $\boldsymbol{\Lambda} = (\boldsymbol{\lambda_1}, \ldots, \boldsymbol{\lambda_d})'$ is the $q \times d$ matrix of factor loadings, where $d << q$. The upper triangular elements of  $\boldsymbol{\Lambda}$ are set to be zero to ensure parameter identifiability. Here we assume that the latent variables follow a multivariate standard normal distribution, $\boldsymbol{b} \sim N(\boldsymbol{0}, \boldsymbol{I})$.

For GLVMs it is important to select initial starting values for the parameters because the observed likelihood may be multimodal, and maximisation algorithms can end up in local maxima. Advancements as described by @niku2019gllvm enable faster and more reliable fits of latent variable models by carefully choosing starting values of the parameters. This has shown to improve speed and increase the accuracy of the estimates. 

A similar method has been implemented in `glmmTMB`. A general linear model is fitted to the data to obtain initial starting values for the fixed parameters in the model.  Residuals from the fitted GLM are calculated; Dunn-Smyth residuals are calculated for common families while residuals from dev.resids function are used otherwise. Initial starting values for the latent variables and their loadings are obtained by fitting a reduced rank model to the residuals.

### Reduced rank

One of our main motivations for adding this variance-covariance matrix is to be able to fit multivariate abundance data, for example to model the abundance of different taxa across multiple sites. Typically an unstructured  random effect is assumed to account for correlation between taxa however the number of parameters required quickly becomes large with increasing taxa. A GLVM is a flexible and more parsimonious way to account for correlation so that it is possible to a fit joint model across many taxa.
...
```{r rr_ex, eval = FALSE}
fit.rr <- glmmTMB(abundance ~ taxa + rr(taxa + 0|site, n = 2))
```

The left hand side of the bar `taxa + 0` corresponds to a factor loading matrix which accounts for the correlation between taxa. The right hand side of the bar splits the above specification independently among sites. The `n` is a non-negative integer (defaults to 2). 

An option in `glmmTMBControl()` has been included to initialise the starting values for the parameters based on the approach mentioned above with the default set at `glmmTMBControl(start_method = list(method = NULL, jitter.sd = 0)`:
\begin{itemize}
	\item `method = "res"` which initialises starting values from the results of fitting a GLM, and fitting a reduced rank model to the residuals to obtain starting values for the fixed coefficients, the latent variables and the factor loadings. 
	\item `jitter.sd` adds variation to the starting values of latent variables when `start = "res"`. Defaults to 0.	
\end{itemize}
