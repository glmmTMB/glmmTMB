...
We use a meta-analysis dataset from the `metafor` package to demonstrate how `equalto` fits a meta-analysis with within-study dependence among effect sizes:


```{r equalto}
suppressWarnings(require("metafor"))
dat <- dat.assink2016
head(dat,9) #view data of the first two studies

dat$g <- 1 # dummy grouping variable
dat$id <- as.factor(1:nrow(dat)) # observation id - this needs to be a factor
...
# equivalent to 
#fit_rma <- rma.mv(yi, VCV, random = list(~1 | study, ~1 | id), control=list(REMLf=FALSE), data=dat)

```


As with `propto`, we need to specify the random term without an intercept so each column maps to a single effect, and ensure the variance-covariance row/column names match and are ordered as the factor levels of the `id` variable. Set a single-level `g` grouping factor if all observations belong to one cluster.
