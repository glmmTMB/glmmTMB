...
=======
 ...
 Equal                            # `equalto`     #  $0$            # Variance-covariance matrix # -
>>>>>>> bde9f6ee (input matrix checks for equalto)
"
)
knitr::kable(ctab)
```
...
# equivalent model in metafor
#fit_rma <- metafor::rma.mv(yi, VCV, random = list(~1 | study, ~1 | id), control=list(REMLf=FALSE), data=dat)
```
...
# equivalent model in metafor
#fit_rma <- metafor::rma.mv(yi, vi, random = list(~1 | study, ~1 | id), control=list(REMLf=FALSE), data=dat)
```
...
The `equalto` structure is experimental - please report any bugs or errors. 
