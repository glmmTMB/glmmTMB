...
A trace plot can be made with the following code, but we won't to save space: 

`rstan::traceplot(m2, pars=c("beta","betazi","theta"))`


A pairs plot can be made with the following code, but we won't to save space: 
...
The trace plot confirms that one of the chains (chain 1) is problematic, but we don't show it to save space.
