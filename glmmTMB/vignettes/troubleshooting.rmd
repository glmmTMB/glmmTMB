...
However, there is a more fundamental problem with this model: count models are being applied to non-integer data! We can try a Tweedie distribution instead:

<!-- FORCE evaluation even on CRAN/override global eval=FALSE -->
```{r mod4_up, eval=FALSE}
...
For what it's worth, the Tweedie is trying pretty hard to get back to a Poisson distribution: the power parameter is `r round(1+plogis(mod4$fit$par[["thetaf"]]),2)`, which is just above the Poisson value of 1 (which in turn is the *minimum* value allowed by glmmTMB).
