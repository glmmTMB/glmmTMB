...
In some cases, scaling predictor variables may help.  For example, in this example from @phisanti, the results of `glm` and `glmmTMB` applied to a scaled version of the data set agree, while `glmmTMB` applied to the raw data set gives a non-positive-definite Hessian warning. (**This is no longer true now that we try harder to compute an accurate Hessian ... we need another example ...**)
...
```{r hackcomp, echo=FALSE, eval=TRUE}
## UGH: have to hack around all of the non-evaluation machinery
zi1 <- -17.333945381341
zi2 <- -11.4800818066051
pzi1 <- 2.96457997868236e-08
pzi2 <- 1.03338896489101e-05
```
