...
> In fitTMB(TMBStruc) : Model convergence problem; non-positive-definite Hessian matrix. See vignette('troubleshooting')

will usually be accompanied by `NA` values for the standard errors, log-likelihood, AIC, and BIC, and deviance. When you run `summary()` on the resulting model, you'll get the warning `In sqrt(diag(vcov)) : NaNs produced`. This warning states that at the (putative) maximum likelihood estimate `glmmTMB` has found, the estimated curvature of the log-likelihood surface is inconsistent with the point really being an MLE: instead, it is upward-curving, or flat, in some direction(s). In technical terms, the matrix of second derivatives of the negative log-likelihood with respect to the parameters (the "Hessian") has zero or negative eigenvalues (or eigenvalues so close to zero that the linear algebra required to compute a variance-covariance matrix from the Hessian fails).

This error is likely to occur:
...
- when a dispersion parameter is effectively zero
...
We can immediately see that the dispersion is very small (`r signif(sigma(mod1),3)`), and that the zero-inflation parameter is strongly negative (`r round(fixef(mod1)[["zi"]],2)`). However, we'll develop some fancier machinery that checks the variance-covariance matrix or Hessian of the model, finds eigenvalues that are negative or close to zero, and identifies which model components contribute to those eigenvalues:
...
This model has a very bad eigenvalue (with a value of `r signif(d1[[1]]$val,3)`) that is mostly driven by the zero-inflation parameter, and a little bit by the dispersion parameter. Let's try dropping the zero-inflation term:
