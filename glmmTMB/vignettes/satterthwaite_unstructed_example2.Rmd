...
```{r, message = FALSE}
library(glmmTMB)
library(mmrm)  # from https://github.com/openpharma/mmrm for the data set
...
```{r}
- 2 * logLik(model0)
all.equal(sas_deviance, - 2 * as.numeric(logLik(model0)))
```
...
- Do we want to make these easier for users to fix/work around?
...
Now this actually does converge. It is surprising that `model0` does not deliver
the same though.

```{r}
- 2 * logLik(model1)
all.equal(- 2 * as.numeric(logLik(model1)), - 2 * as.numeric(logLik(model0)))
```
...
This model can also be fitted with `lme4` and `lmerTest` as follows:
...
all.equal(sas_beta$est, as.numeric(beta_model0))
all.equal(beta_model0, beta_model1)
all.equal(beta_model1, beta_model2)
all.equal(beta_model2, beta_model2_lmerTest)
```
...
all.equal(sas_cov, cov_model0, check.attributes = FALSE)
all.equal(cov_model0, cov_model1, check.attributes = FALSE)
all.equal(sas_cov, cov_model1, check.attributes = FALSE)
all.equal(cov_model1, cov_model2, check.attributes = FALSE)
all.equal(sas_cov, cov_model2, check.attributes = FALSE)
all.equal(cov_model0, cov_model2, check.attributes = FALSE)
all.equal(cov_model2, cov_model2_lmerTest, check.attributes = FALSE)
all.equal(sas_cov, cov_model2_lmerTest, check.attributes = FALSE)
```

It is surprising that `model0` and `model1` don't yield the same covariance
estimate. Both fix the residual variance at a constant, but then the remaining
variance parameters should compensate accordingly.

There are some numerical differences, but fortunately the SAS result is close
to the `model0` result (slightly more than `1e-4`).
...
get_df <- function(model) {
  model_theta_vcov <- vcov(model, full = TRUE)[-(1:11), -(1:11)]
  ...
  all.equal(unname(model_beta_vcov), model_covbeta_theta(model_theta_est))
