...
this for the different models from above. We hope for the `model0_optim` from above 
since it worked best so far (it converged using `optim`
with the automatic differentiation and gave the closest covariance matrix).
...
```{r satt_funs}
get_df <- function(model, L, vcov_tolerance = 1e-4) {
  stopifnot(is(model, "glmmTMB"))
  ...
  stopifnot(all.equal(
    unname(model_beta_vcov), 
    model_covbeta_theta(model_theta_est),
    tolerance = vcov_tolerance
  ))
...
Problem: the residual variance $\sigma^2$ appears in both the numerator and 
denominator. 
We can try to take out the residual variance manually so that we don't have to
depend on the limiting behavior.

Let's try this on the different models:

**fixme/caution**: if not controlling number of OpenMP threads 
(e.g. `export OMP_NUM_THREADS=1` in parent shell), this can go crazy ...

```{r calc_satt, cache=TRUE}
L <- c(1, rep(0, 10)) # get the intercept only.
(df_model0 <- get_df(model0, L, vcov_tolerance = 1e-3)) # gives 0.01316441
(df_model0_optim <- get_df(model0_optim, L)) # gives 0.09962662
(df_model1 <- get_df(model1, L)) # gives 167.8286
...
0) Why is the Satterthwaite in `model0` completely off? Is it because of the
  ...
  -> It is not because of "false convergence", since also `model0_optim` is 
     completely off.
  -> How can we improve the function `model_covbeta_theta()`?

1) Can we set sigma exactly to zero to improve this? (by modifying `glmmTMB`) 
  -> See the feature branch `zero_resids`.

2) Why do we get "false convergence" with the default little epsilon used for $\sigma$
   in `model0`?
  -> I would guess it is because the very small `sigma` used there.

3) Could we potentially perform the Jacobian calculation with (another version of) `TMB`?
  (so that we don't have to use numeric differentiation but can actually use
   automatic differentiation).
  -> Yes, see below

### Another way to get `model_covbeta_theta()`

Brainstorming:

- `sdreport`: After optimization of an AD model, `sdreport` is used to calculate standard deviations of all model parameters, *including non linear functions of random effects and parameters* specified through the `ADREPORT()` macro from the user template.
-> not useful I guess

- Manual covariance matrix: We could try to manually calculate the
covariance matrix, based on the marginal model. We would just
need to know where in `glmmTMB` object the design matrices are
for the random and fixed effects etc.
-> not so interesting since it does not address the core problem.

- Cov matrix: Can the new AD library `TMBad` help potentially? This is a new AD framework (previously `CppAD`). See `?TMB::compile`. From https://github.com/glmmTMB/glmmTMB/issues/803 it seems to me that the AD hessian of the log-likelihood function could be helpful? As this would give more a closed form representation of the covariance matrix?
-> maybe interesting

- Jacobian: In principle `TMB` supports Jacobian calculations, see 
https://github.com/kaskr/adcomp/blob/d6bb3f238ddee243745658e331d07853a3c9d673/TMB/inst/include/tmbutils/autodiff.hpp#L143 ff.
-> Promising! We could also ask Kasper Kristensen for help with this.

- I wonder if trying out a simple `TMB`-only prototype of the 
unstructured covariance model could be instructive? (so basically
bare-bones without nice R interface etc.)
-> It could be nice to have vector-valued random effects example for TMB anyway...
and we could then try to add the Jacobian calculation on top. That should be instructive
before trying to put this together with `glmmTMB`.
