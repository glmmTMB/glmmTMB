...
## Extracting coefficients, coefficient plots and tables
...
```{r results="asis"}
ss <- summary(owls_nb1)
xtable(lme4::formatVC(ss$varcor$cond))
xtable(coef(ss)$cond)
xtable(coef(ss)$zi)
```
...
```{r texreg_def}
if (suppressMessages(require("texreg"))) {
  have_texreg <- TRUE
  source(system.file("other_methods","extract.R",package="glmmTMB"))
} 
```

then `texreg(owls_nb1)` produces:

```{r texreg_show, results="asis", echo=FALSE}
if (!have_texreg) {
  cat("\text{texreg package isn't installed, can't illustrate}\n")
} else {
  texreg(owls_nb1)
}
```
