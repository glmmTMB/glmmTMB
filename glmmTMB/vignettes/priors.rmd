...
knitr::opts_chunk$set(eval = all(sapply(c("purrr", "blme", "broom.mixed", "dplyr", "ggplot2", "pscl"), require,
                                        character.only = TRUE)),
...
We can also use priors on the zero-inflation coefficients, either (1) to mitigate complete separation or (2) in the case where the zero-inflation probability goes to zero. In the latter case, even with an intercept-only zero-inflation model (`ziformula = ~1`), the parameter on the log-odds (link) scale goes to $-\infty$, the log-likelihood surface becomes very flat, and we end up with non-positive-definite Hessian problems and/or failure of the Wald approximation.
...
cprior <- data.frame(prior = rep("normal(0,3)",2),
                     ...
                     coef = c("(Intercept)", ""))
zi_fit_prior <- update(zi_fit, priors = cprior)
```


```{r zifit_plot, echo = FALSE}
zres <- (purrr::map_dfr(list(basic = zi_fit,
                             prior = zi_fit_prior),
                        ~ tidy(., conf.int = TRUE,
                               component = "zi",
                               effects = "fixed"),
...
yrlabs <- 1800+head(as.numeric(unique(prussian$year)), -1)
ggplot(zres, aes(estimate, term, colour = model)) +
  geom_pointrange(aes(xmin = lwr, xmax = upr),
                  position = position_dodge(width = 0.25)) +
  coord_cartesian(xlim = c(-10, 10)) +
  scale_y_discrete(labels = c("intercept", yrlabs)) + 
  scale_colour_manual(values = OkIt) +
  labs(y = "year")
```
...
- allow multivariate (joint) priors on parameter vectors rather than iid priors?
     - esp for correlation matrices: LKJ, Wishart etc. (from Mikael Jagan)
