...
\begin{itemize}
  \item response distributions: Gaussian, binomial, beta-binomial, Poisson, negative binomial (NB1 and NB2 parameterizations), Conway-Maxwell-Poisson, generalized Poisson, Gamma, Beta, Tweedie; as well as zero-truncated Poisson, Conway-Maxwell-Poisson, generalized Poisson, and negative binomial; \notimpl{Student $t$} See \code{?family\_glmmTMB} for a current list including details of parameterizations.
...
## lm_nowarn <- function(...) suppressWarnings(lm(...))
suppressWarnings(
...
ggplot(tmatContraception, aes(n, time, colour=pkg)) + geom_point() +
  ...
  geom_smooth(method="lm_nowarn", formula = y ~ x) +
