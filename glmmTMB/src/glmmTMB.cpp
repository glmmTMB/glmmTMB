...
#define zt_thresh 0.001
#define zt_lik(x,loglik_exp) (zi_flag && (x < Type(zt_thresh)) ? -INFINITY : loglik_exp)
  ...
  for (int i=0; i < yobs.size(); i++){
    if ( !glmmtmb::isNA(yobs(i)) ) {
	  ...
	  tmp_loglik = zt_lik(yobs(i), tmp_loglik);
          SIMULATE{
            s1 = mu(i)/phi(i);//sz
            s2 = 1/(1+phi(i)); //pb
