...
test_that("Basic Binomial CBPP examples", {
    ...
    expect_is(gm0 <- glmmTMB(cbind(incidence, size-incidence) ~ 1 + (1|herd),
                             data = cbpp, family=binomial), "glmmTMB")
    expect_is(gm1 <- glmmTMB(cbind(incidence, size-incidence) ~ 1 + period + (1|herd),
                             data = cbpp, family=binomial), "glmmTMB")

    expect_equal(fixef(gm0), c("(Intercept)" = -2.04567), tolerance = .0001)
    expect_equal(fixef(gm1), c("(Intercept)" = -1.39834,
                               period2 = -0.991925, period3 = -1.12822, period4 = -1.57975),
                  tolerance = .001) # <- TODO: lower eventually
})


test_that("Update Binomial", {
    expect_equal(gm1, update(gm0, . ~ . + period))
})
